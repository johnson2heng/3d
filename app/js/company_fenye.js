(function($) {
	...
	let Company_fen = (function() {
		//方法的构造函数  
		function Company_fen(element, options) {
			this.settings = options;
			this.element = element;
			this.init();
		}
		//方法的原型方法  
		Company_fen.prototype = {
			init: function() {
				var me = this;
						...
						<div class="container-fluid">
							<div class="navbar-header">
								<a class="navbar-brand" href="#">
									关注的企业共 <span class="shu" style="color: green;"> ${src.length}&nbsp;</span> 个
								</a>
							</div>
				...
				for(let i = start; i < stop; i++) {
					html += `
            			<div class="col-xs-4">
							...
							<div>
								<div class="span25" style="width:33%;">
			...
			add_con: function(wrap, library) {
				//给列表添加查看详情事件
				add_company_fun(null, wrap, null,library);
			},
			/*为按钮添加事件*/
			add_list: function() {
				var me = this;
		...
		return Company_fen; //一定要return，要不然无法调用构造函数  
	})();

	$.fn.Company_fen = function(options) {
		return this.each(function() {
			let me = $(this);
			let	instance = $(me).data("Company_fen");
			let option = $.extend($.fn.Company_fen.defaults, options || {});
			//判断是否实例化，如果没有，则创建实例  
			if(!instance) {
				me.data("Company_fen", (instance = new Company_fen(me, option)));
			}
			//new Company_fen(me,options);
			//根据传入的数据类型，可以调用实例里面的方法  
			if($.type(options) === "string") return instance[options]();
		});
	};

	$.fn.Company_fen.defaults = {
		//此处书写默认值  
